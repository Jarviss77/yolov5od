...
def run(
    ...
    csv_path = save_dir / "frame_labels.csv"
   ...
   # Load model
    ...
    def write_labels_to_csv(labels):
        with open(csv_path, mode='w', newline='') as file:
            ...
            for frame_num, label_list in enumerate(labels):
                writer.writerow([frame_num, ", ".join(label_list)])

    # Function to check if a given input string exists in the labels of any frame
    def check_label_exists(labels, input_string):
        return any(input_string in frame_labels for frame_labels in labels)
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        class_instance_counters = {name: 0 for name in names}

        # Initialize the dictionary without any pre-defined keys
        ...
        for i, det in enumerate(pred):  # per image
            ...
            if len(det):
                ...
                for *xyxy, conf, cls in reversed(det):
                    ...
                    coordinates = tuple(xyxy)  # Bounding box coordinates
                    confidence = float(conf)
                    confidence_str = f"{confidence:.2f}"

                    # # Increment instance counter for the class dynamically
                    # if label not in class_instance_counters:
                    #     class_instance_counters[label] = 0
                    # class_instance_counters[label] += 1
                    # unique_instance_id = class_instance_counters[label]
                    if class_name not in class_instances:
                        class_instances[class_name] = []

                        # Create a unique ID for each instance based on the number of existing instances
                    instance_id = len(class_instances[class_name]) + 1

                    # Store the instance details in a dictionary
                    instance_details = {
                        "id": instance_id,
                    ...
                    if save_txt:  # Write to file
                        ...
                        line = (cls, *xywh, confidence) if save_conf else (cls, *xywh)  # label format
